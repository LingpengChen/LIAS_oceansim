...
# from isaacsim.ros1.bridge import RosBridge
# from isaacsim.ros1.bridge import register_node_writer_with_telemetry
...
class UW_Camera(Camera):
    ...
    def __init__(self, 
                ...
                #  resolution=[1280, 720],
                 resolution=[Cam_image_width, Cam_image_height],
                 position = None, 
                 orientation = None, 
                 translation = None, 
    ...
    def initialize(self, 
        ...
        # 1. 设置相机的光学属性 (内参)
        self.set_focal_length(Focal_length_mm) # focal_length_mm
        self.set_horizontal_aperture(Horizontal_aperture_mm) # horizontal_aperture_mm

 
        horizontal_fov = self.get_horizontal_fov()
        intrisics = self.get_intrinsics_matrix()
        print(f"Horizontal Fov is {horizontal_fov}")
        print(f"intrisics: {intrisics}")
    ...
    def render(self):
        ...
        if raw_rgba.size !=0:
            ...
            # uw_image_np = uw_image.numpy()  # warp array to numpy
            uw_image_np = uw_image.numpy()  # warp array to numpy
            uw_image_rgb = uw_image_np[..., :3]  # RGBA to RGB
    ...
    def make_viewport(self):
        ...
        self.window = ui.Window(self._name, width=640, height=480 + 20, visible=True)
        self._provider = ui.ByteImageProvider()
        with self.window.frame:
            with ui.ZStack(height=480):
                ui.Rectangle(style={"background_color": 0xFF000000})
                ui.Label('Run the scenario for image to be received',
                         style={'font_size': 55,'alignment': ui.Alignment.CENTER},
                         word_wrap=True)
                image_provider = ui.ImageWithProvider(self._provider, width=640, height=480,
