...
# Isaac sim import
from isaacsim.core.prims import SingleRigidPrim, SingleGeometryPrim, XFormPrim
...
class UIBuilder():
    ...
    def build_ui(self):
        ...
        with world_controls_frame:
            with ui.VStack(style=get_style(), spacing=5, height=0):
                ...
                # 你点击“LOAD”按钮，set_world_settings会设置物理和渲染步长。
                # 仿真运行时，物理引擎每隔physics_dt秒推进一次。
                # 每推进一次，就会调用_update_scenario(step)，其中step就是你设置的physics_dt。
                
                self._load_btn = LoadButton(
                    "Load Button", "LOAD", setup_scene_fn=self._setup_scene, setup_post_load_fn=self._setup_scenario
                )
                self._load_btn.set_world_settings(physics_dt=PHYSICS_DT, rendering_dt=RENDERING_DT)
    ...
    def _on_init(self):
        ...
        # Robot parameters
        self._rob_mass = ROB_MASS # kg
        self._rob_angular_damping = 3.0
        self._rob_linear_damping = 3.0
    ...
    def _setup_scene(self):
        ...
        if self._USD_path_field.get_value_as_string() != "":
            ...
            # scene_prim_path = '/World/scene'
            # scene_usd_path = get_oceansim_assets_path() + "/collected_MHL/mhl_water.usd"
            # add_reference_to_stage(usd_path=scene_usd_path, prim_path=scene_prim_path)    

            # scene_prim_mesh_path = scene_prim_path + '/mhl_scaled'
            # # apply a reflectivity of 1.0 to mesh of the scene for sonar simulation
            # add_update_semantics(prim=get_prim_at_path(scene_prim_mesh_path + "/Mesh/mesh"),
            ...
            # # Toggle rigid body for the rock  /World/scene/mhl_scaled/Mesh/mesh
            ...
            # # apply a reflectivity of 1.0 to mesh of the scene for sonar simulation
            # add_update_semantics(prim=get_prim_at_path(scene_prim_mesh_path), # /World/scene/seafloor/stone /World/scene/seafloor/terrainstone/node_7_009/Mesh_018
            #                     type_label='reflectivity', 
            #                     semantic_label='1.0')
            # apply_semantics_to_all_meshes_under_path(root_path=scene_prim_mesh_path,
            #                                         type_label='reflectivity', 
            #                                          semantic_label='1.0')

            # scene_prim_mesh_path = scene_prim_path + '/terrainstone'
            # # apply a reflectivity of 1.0 to mesh of the scene for sonar simulation
            # apply_semantics_to_all_meshes_under_path(root_path=scene_prim_mesh_path,
            #                                         type_label='reflectivity', 
            #                                          semantic_label='1.0')
    
            # scene_prim_mesh_path = scene_prim_path + '/sand'
            # # apply a reflectivity of 1.0 to mesh of the scene for sonar simulation
            # apply_semantics_to_all_meshes_under_path(root_path=scene_prim_mesh_path,
            #                                         type_label='reflectivity', 
            #                                          semantic_label='2.0')
        ...
        robot_pose = INITIAL_T
        SingleRigidPrim(prim_path=robot_prim_path,
                        mass=self._rob_mass,
                        translation=robot_pose)
        cam_pose = robot_pose + np.array([1, 1, 1])
        set_camera_view(eye=cam_pose, target=rob_collider_prim.get_world_pose()[0])
        ...
        if self._use_sonar:
            ...
            self._sonar = ImagingSonarSensor(prim_path=robot_prim_path + '/sonar',
                                            translation=Sonar_trans,
                                            orientation=Sonar_orientation,
                                            hori_res=4000
                                            )
        ...
        if self._use_camera:
            ...
            self._cam.set_clipping_range(0.1, CAM_RANGE)
            
            self._cam2 = UW_Camera(prim_path=robot_prim_path + '/UW_camera2',
                                    orientation=Cam2_orientation,
                                    translation=Cam2_trans)
            
            self._cam2.set_clipping_range(0.1, CAM_RANGE)
    ...
    def _reset_scenario(self):
        ...
        self._scenario.setup_scenario(self._rob, self._sonar, self._cam, self._cam2, self._DVL, self._baro, self._ctrl_mode, self._use_IMU)
...
def apply_semantics_to_all_meshes_under_path(root_path: str, type_label: str, semantic_label: str):
    """
    遍历指定路径下的所有图元，如果图元是Mesh类型，则为其添加或更新语义标签。

    Args:
        root_path (str): 要开始搜索的根图元路径，例如 "/World/MyScene/Props"。
        type_label (str): 要应用的语义类型标签，例如 "class"。
        semantic_label (str): 要应用的具体语义标签，例如 "prop"。
    """
    # 1. 获取当前的USD Stage
    stage = omni.usd.get_context().get_stage()
    if not stage:
        print("Error: Could not get the USD stage.")
        return

    # 2. 获取根图元
    root_prim = stage.GetPrimAtPath(root_path)
    if not root_prim.IsValid():
        print(f"Error: The specified root path '{root_path}' is not valid or does not exist.")
        return

    print(f"Starting to search for meshes under: {root_path}")
    
    # 3. 遍历根图元下的所有子孙图元
    # Usd.PrimRange 是一个高效的迭代器，可以遍历一个图元及其所有后代
    for prim in Usd.PrimRange(root_prim):
        # 4. 检查图元的类型是否为 Mesh
        # UsdGeom.Mesh 是所有网格物体的基础类型
        if prim.IsA(UsdGeom.Mesh):
            try:
                ...
                print(f"  -> Applied semantics to mesh: {prim.GetPath()}")
            except Exception as e:
                print(f"  -> Failed to apply semantics to {prim.GetPath()}: {e}")

    print("Finished applying semantics.")


# # --- 执行示例 ---

# # 请在这里修改为你自己的路径和参数
# ROOT_PATH_TO_SEARCH = "/World/MyScene"  # <<<--- 修改这里！这是你的 scene_prim_mesh_path
# SEMANTIC_TYPE = "reflectivity"
# SEMANTIC_LABEL = "2.0"

# # 调用函数
# apply_semantics_to_all_meshes_under_path(ROOT_PATH_TO_SEARCH, SEMANTIC_TYPE, SEMANTIC_LABEL)
